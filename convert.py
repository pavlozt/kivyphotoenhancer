...
class ConvertApp(App):
    ...
    def do_upload(self):
        ...
        self.convert_thread = threading.Thread(target=self.convert, 
                        args=(self.root.ids.droparea.filenamelist)
                    )
    ...
    # запускается в отдельном треде, но для правильного обновления элементов интерфейса, нужно вызывать декорированные mainthread
    def convert(self,*filelist):
        ...
        for fullname in filelist :
            ...
            subprocess.call([imagickexe,fullname,'-colorspace', 'Lab','-channel',str(0),'-auto-level','+channel','-colorspace', 'sRGB','out/'+filename])
            p += 1
